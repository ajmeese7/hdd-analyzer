...
3. Create the GitHub release with only this version's changelog section as the notes (never the whole file); publishing it triggers the workflow, which runs the tests, builds the wheel and sdist, and uploads them to PyPI:

```
uv run python scripts/release-notes.py X.Y.Z > notes.md
gh release create vX.Y.Z --title "hdd-analyzer X.Y.Z" --notes-file notes.md
rm notes.md
```
